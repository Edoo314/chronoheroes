...
for version in ["V2", "V3"]:
    filename = f"ch_events_{version}.csv"
    print(f"\n=== Import {filename} ===")
    try:
        with open(filename, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if not person_id:
                    print(f"  ERREUR personne: {row['person_name']}")
                    continue
                status = insert_event(row, person_id)
                if status in [200, 201]:
                    total_ok += 1
                    print(f"  OK  {row['person_name']} — {row['description_fr'][:50]}")
                elif status == 409:
                    total_skip += 1
                else:
                    print(f"  ERR {status} {row['person_name']}")
                time.sleep(0.1)
    except FileNotFoundError:
        print(f"  Fichier {filename} introuvable")

print(f"\nTermine: {total_ok} evenements importes / {total_skip} doublons ignores")
